...
function Scene({progress}){return <Canvas dpr={[1,1.7]} gl={{antialias:true,alpha:false}} shadows><PerspectiveCamera makeDefault position={[4.7,2.15,6.25]} fov={36}/><color attach="background" args={['#050505']}/><ambientLight intensity={.6}/><hemisphereLight intensity={1.1} groundColor="#080808" skyColor="#ffffff"/><spotLight position={[4,6,5]} intensity={90} angle={.5} penumbra={1} castShadow/><spotLight position={[-5,3,-4]} intensity={70} angle={.55} penumbra={1}/><pointLight position={[0,2,3]} intensity={25}/><CinematicLights progress={progress}/><Suspense fallback={<CarFallback/>}><Float speed={1.05} rotationIntensity={.025} floatIntensity={.12}><RealCar progress={progress}/></Float></Suspense><ContactShadows position={[0,0,0]} opacity={.55} scale={15} blur={2.8} far={7}/><Environment preset="studio"/></Canvas>}
